...
if ! command -v tailscale &> /dev/null; then
    echo '{"text":"ERR","class":"not-installed","tooltip":"Tailscale: NOT INSTALLED"}'
    exit 0
fi
...
if [ $? -ne 0 ]; then
    echo '{"text":"ERR","class":"error","tooltip":"Tailscale: ERROR\nFailed to get status"}'
    exit 0
fi
...
if [ "$backend_state" == "Running" ]; then
    ...
    account_short=$(echo "$account" | cut -d'@' -f1 | head -c 3 | tr '[:lower:]' '[:upper:]')
    ...
    echo '{"text":"OFF","class":"disconnected","tooltip":"Tailscale: STOPPED"}'
else
    echo "{\"text\":\"UNK\",\"class\":\"unknown\",\"tooltip\":\"Tailscale: UNKNOWN STATE\n$backend_state\"}"
fi
